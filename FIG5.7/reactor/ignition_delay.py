...
pkl.dump(ignition_delay,open('ign_del.pkl','wb'))
pkl.dump(i,open('temperature_ign.pkl','wb'))
plt.semilogy(i, ignition_delay, label='Reactor Chain')
plt.xlabel('ignition delay time [s]')
plt.ylabel('$preheating temperature$ [K]')
plt.legend(loc=0)
plt.show()
